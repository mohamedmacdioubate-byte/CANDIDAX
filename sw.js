...
// Fetch
// - Pages HTML (navigation) : réseau en priorité pour rester à jour,
//   secours sur le cache si hors ligne (app.html reste app.html hors ligne,
//   pas de redirection vers l'accueil).
// - API IA / fournisseurs externes : réseau uniquement, jamais de cache.
// - Reste des assets (CSS, polices, icônes) : cache en priorité, réseau en secours.
self.addEventListener('fetch', function(e) {
  var req = e.request;
  var url = req.url;

  if (url.includes('/api/') || url.includes('api.anthropic.com') || url.includes('api.openai.com') || url.includes('supabase.co')) {
    ...
    e.respondWith(
      fetch(req).then(function(response) {
        ...
        return caches.match(req).then(function(cached) {
          ...
          if (url.indexOf('app.html') !== -1) return caches.match('/app.html');
